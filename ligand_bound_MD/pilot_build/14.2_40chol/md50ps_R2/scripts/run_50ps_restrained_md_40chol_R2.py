...
INPUT_PDB = PILOT_ROOT / "short_md_R2" / "outputs" / "TRKB_40chol_L002_14.2_10ps_R2_final.pdb"
CHECKPOINT_IN = PILOT_ROOT / "short_md_R2" / "outputs" / "TRKB_40chol_L002_14.2_10ps_R2.chk"
STATE_XML_IN = PILOT_ROOT / "short_md_R2" / "outputs" / "TRKB_40chol_L002_14.2_10ps_R2_state.xml"
R2_MINIMIZED_INPUT_PDB = MINIMIZED_PDB

LOG_PATH = LOGS_DIR / "md50ps_40chol_R2.log"
MONITOR_CSV = REPORTS_DIR / "md50ps_40chol_R2_monitor.csv"
SUMMARY_MD = REPORTS_DIR / "md50ps_40chol_R2_summary.md"
FINAL_PDB = OUTPUTS_DIR / "TRKB_40chol_L002_14.2_50ps_R2_final.pdb"
DCD_PATH = OUTPUTS_DIR / "TRKB_40chol_L002_14.2_50ps_R2.dcd"
STATE_XML = OUTPUTS_DIR / "TRKB_40chol_L002_14.2_50ps_R2_state.xml"
CHECKPOINT_PATH = OUTPUTS_DIR / "TRKB_40chol_L002_14.2_50ps_R2.chk"
...
def monitor_row(
    ...
    return {
        ...
        "ligand_heavy_rmsd_vs_50ps_input_A": f"{rmsd_vs_input:.6g}",
...
def write_summary(result: dict[str, Any], rows: list[dict[str, Any]]) -> None:
    ...
    lines = [
        ...
        "## Outcome",
        "",
        f"- Completed 50 ps: {result.get('completed_50ps')}",
        ...
        f"- Ligand RMSD vs 50 ps input A initial/final/max: {fmt(rmsd['initial'])}/{fmt(rmsd['final'])}/{fmt(rmsd['max'])}",
        "",
        "## Clash Monitoring",
        "",
        ...
        "## Recommendation",
        "",
        f"- Recommend next 100 ps 40chol restrained MD: {result.get('recommend_100ps')}",
        "- This stage is still restrained MD pilot work, not production MD.",
    ]
    SUMMARY_MD.write_text("\n".join(lines) + "\n", encoding="utf-8")


def initialize_state(simulation: Any, pdb: Any, integrator: Any) -> tuple[bool, bool, str]:
    ...
    simulation.context.setVelocitiesToTemperature(TEMPERATURE_K * kelvin, 142450)
    return False, True, reason


def run_md50ps() -> dict[str, Any]:
    import openmm
    ...
    reference_50ps = positions_to_angstrom(pdb.positions)
    reference_minimized = read_reference_coords(R2_MINIMIZED_INPUT_PDB)

    print("50 ps R2 restrained MD pilot for 14.2 / L002 / 40chol")
    ...
    integrator.setRandomNumberSeed(142450)
    simulation = Simulation(psf.topology, system, integrator, platform)
    continued_from_checkpoint, fallback_used, fallback_reason = initialize_state(simulation, pdb, integrator)
    ...
    def sample(force_write: bool) -> bool:
        ...
        if finite:
            ...
            rmsd_vs_input = ligand_rmsd(atoms, reference_50ps, coords)
        ...
        if force_write or abort:
            ...
            print(
                ...
                "RMSD50 {rmsd50:.3f} A RMSDmin {rmsdmin:.3f} A severe/close {severe}/{close} status {status}".format(
                    step=global_step,
                    time=row["time_ps"],
                    temp=temperature,
                    pe=potential,
    ...
    write_pdb(
        ...
        FINAL_PDB,
        [
            "REMARK 40chol 50 ps R2 restrained MD pilot final safe coordinates",
    ...
    recommend_100ps = (
        ...
        and not has_nan
        and last_row.get("status") in {"ok", "warning_SER20_distance_gt_10A"}
    ...
    print(f"completed 50 ps: {'YES' if completed else 'NO'}")
    ...
    print(f"ligand final RMSD: {last_row.get('ligand_heavy_rmsd_vs_50ps_input_A', 'nan')}")
    ...
    print(f"recommend next 100 ps 40chol restrained MD: {'YES' if recommend_100ps else 'NO'}")
    ...
    with LOG_PATH.open("w", encoding="utf-8") as log_file:
        ...
        with redirect_stdout(tee_out), redirect_stderr(tee_err):
            try:
                ...
                error = repr(exc)
                print("FAILED: 40chol 50 ps R2 restrained MD failed:", error)
                traceback.print_exc()
