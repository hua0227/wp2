...
LOG_PATH = LOGS_DIR / "short_md_40chol_R2.log"
MONITOR_CSV = REPORTS_DIR / "short_md_40chol_R2_monitor.csv"
SUMMARY_MD = REPORTS_DIR / "short_md_40chol_R2_summary.md"
FINAL_PDB = OUTPUTS_DIR / "TRKB_40chol_L002_14.2_10ps_R2_final.pdb"
DCD_PATH = OUTPUTS_DIR / "TRKB_40chol_L002_14.2_10ps_R2.dcd"
STATE_XML = OUTPUTS_DIR / "TRKB_40chol_L002_14.2_10ps_R2_state.xml"
CHECKPOINT_PATH = OUTPUTS_DIR / "TRKB_40chol_L002_14.2_10ps_R2.chk"
...
MONITOR_FIELDS = [
    ...
    "total_energy_kJ_mol",
    "ligand_min_distance_TYR13_A",
    "ligand_min_distance_VAL17_A",
    "ligand_min_distance_SER20_A",
...
def monitor_row(
    ...
    return {
        ...
        "ligand_min_distance_TYR13_A": f"{key_distances['TYR13']:.6g}",
        "ligand_min_distance_VAL17_A": f"{key_distances['VAL17']:.6g}",
        "ligand_min_distance_SER20_A": f"{key_distances['SER20']:.6g}",
        "ligand_key_min_distance_A": f"{key_distances['key_min']:.6g}",
...
def write_summary(result: dict[str, Any], rows: list[dict[str, Any]]) -> None:
    ...
    tyr = summarize_numeric(rows, "ligand_min_distance_TYR13_A")
    val = summarize_numeric(rows, "ligand_min_distance_VAL17_A")
    ser = summarize_numeric(rows, "ligand_min_distance_SER20_A")
    key = summarize_numeric(rows, "ligand_key_min_distance_A")
    rmsd = summarize_numeric(rows, "ligand_heavy_rmsd_A")
    ...
    lines = [
        ...
        "## Outcome",
        "",
        f"- Completed 10 ps: {result.get('completed_10ps')}",
        ...
        f"- Potential energy kJ/mol initial/final/min/max/mean: {fmt(pe['initial'])}/{fmt(pe['final'])}/{fmt(pe['min'])}/{fmt(pe['max'])}/{fmt(pe['mean'])}",
        "",
        "## Ligand Retention",
        "",
        ...
        f"- Ligand heavy-atom RMSD A initial/final/max: {fmt(rmsd['initial'])}/{fmt(rmsd['final'])}/{fmt(rmsd['max'])}",
        "",
        "## Clash Monitoring",
        "",
        ...
        "## Recommendation",
        "",
        f"- Recommend next 50 ps 40chol restrained MD pilot: {result.get('recommend_50ps')}",
...
def run_short_md() -> dict[str, Any]:
    ...
    ensure_dirs()
    if sum(stage.steps for stage in STAGES) != TOTAL_STEPS:
        raise ValueError("Stage steps do not sum to TOTAL_STEPS")
    ...
    print("10 ps R2 restrained MD pilot for 14.2 / L002 / 40chol")
    ...
    integrator.setRandomNumberSeed(142410)
    simulation = Simulation(psf.topology, system, integrator, platform)
    simulation.context.setPositions(pdb.positions)
    simulation.context.setVelocitiesToTemperature(50.0 * kelvin, 142410)
    ...
    stopped = False
    stop_reason = ""
    key_warning_streak = 0
    severe_warning_streak = 0
    global_step = 0

    def sample(stage_name: str, target_temperature: float, force_write: bool) -> bool:
        nonlocal last_positions, last_row, has_nan, stop_reason, key_warning_streak, severe_warning_streak
        ...
        if finite:
            ...
            rmsd = ligand_rmsd(atoms, reference_coords, coords)
        ...
        if chains == 0:
            ...
            abort = True
        if math.isfinite(key_distances["key_min"]) and key_distances["key_min"] > KEY_DISTANCE_WARNING_A:
            statuses.append("warning_key_distance_gt_10A")
            key_warning_streak += 1
            if key_warning_streak >= 5:
                statuses.append("persistent_key_distance_warning")
                abort = True
        else:
            key_warning_streak = 0
        ...
        if force_write or abort:
            rows.append(row)
            print(
                "step {step} time {time} ps stage {stage} temp {temp:.3f} K PE {pe:.6g} "
                "TYR/VAL/SER {tyr:.3f}/{val:.3f}/{ser:.3f} A chains {chains} RMSD {rmsd:.3f} A "
                "severe/close {severe}/{close} status {status}".format(
                    step=global_step,
                    time=row["time_ps"],
                    stage=stage_name,
                    temp=temperature,
    ...
    for stage in STAGES:
        ...
        while remaining > 0:
            ...
            if sample(stage.name, stage.target_temperature_k, force_write):
                stopped = True
    ...
    write_pdb(
        ...
        FINAL_PDB,
        [
            "REMARK 40chol 10 ps R2 restrained MD pilot final safe coordinates",
    ...
    rmsd_stats = summarize_numeric(rows, "ligand_heavy_rmsd_A")
    recommend_50ps = (
        completed
        and not has_nan
        and last_row.get("status") == "ok"
        ...
        and to_float(last_row.get("ligand_key_min_distance_A")) <= RETENTION_CUTOFF_A
    ...
    print(f"completed 10 ps: {'YES' if completed else 'NO'}")
    ...
    print(
        "ligand final min distance to TYR13/VAL17/SER20: "
        f"{last_row.get('ligand_min_distance_TYR13_A', 'nan')}/"
        f"{last_row.get('ligand_min_distance_VAL17_A', 'nan')}/"
        f"{last_row.get('ligand_min_distance_SER20_A', 'nan')}"
    )
    print(f"ligand final near chains count: {last_row.get('ligand_near_chains_count', 'nan')}")
    print(f"ligand final RMSD: {last_row.get('ligand_heavy_rmsd_A', 'nan')}")
    ...
    print(f"recommend next 50 ps 40chol restrained MD: {'YES' if recommend_50ps else 'NO'}")
    ...
    with LOG_PATH.open("w", encoding="utf-8") as log_file:
        ...
        with redirect_stdout(tee_out), redirect_stderr(tee_err):
            try:
                ...
                error = repr(exc)
                print("FAILED: 40chol 10 ps R2 restrained MD failed:", error)
                traceback.print_exc()
