...
SCRIPT = Path(__file__).with_name("run_100ps_restrained_md_40chol_R2.py")


def load_module():
    spec = importlib.util.spec_from_file_location("run_100ps_restrained_md_40chol_R2", SCRIPT)
...
def test_100ps_step_budget_and_reporting_are_bounded():
    module = load_module()
    assert module.TIMESTEP_PS == 0.0005
    assert module.TOTAL_STEPS == 200000
    assert module.TOTAL_STEPS * module.TIMESTEP_PS == 100.0
    assert module.REPORT_INTERVAL_STEPS == 2000
    assert module.DCD_INTERVAL_STEPS == 4000


def test_monitor_fields_track_each_key_residue_and_two_rmsds():
    module = load_module()
    assert "ligand_distance_TYR13_A" in module.MONITOR_FIELDS
    assert "ligand_distance_VAL17_A" in module.MONITOR_FIELDS
    ...
    assert "ligand_heavy_rmsd_vs_100ps_input_A" in module.MONITOR_FIELDS
