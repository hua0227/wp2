...
LIGAND_ID = os.environ.get("TRKB_LIGAND_ID", "").strip()
RESNAME = os.environ.get("TRKB_RESNAME", "").strip()
SEGID = "LIG"
CHAIN_ID = "Z"
LIGAND_RESID = "1"
...
def require_ligand_context() -> None:
    if not LIGAND_ID or not RESNAME:
        raise RuntimeError("TRKB_LIGAND_ID and TRKB_RESNAME environment variables must be set before importing generic_pilot_common.py")


def seed_for(system_name: str) -> int:
    digits = "".join(ch for ch in LIGAND_ID if ch.isdigit())
    base = int(digits) if digits else 0
    return base * 1000 + (20 if system_name == "20chol" else 40)


def make_config(system_name: str) -> PilotConfig:
    require_ligand_context()
...
def run_pipeline(config: PilotConfig) -> dict[str, Any]:
    ...
    try:
        ...
        result["preflight_ok"] = bool(preflight["parameter_load_ok"] and preflight["atom_count_matches"] and preflight["ligand_pose_exists"] and preflight["ligand_str_exists"])
