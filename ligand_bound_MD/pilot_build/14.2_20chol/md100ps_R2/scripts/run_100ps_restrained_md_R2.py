...
INPUT_PDB = PILOT_ROOT / "md50ps_R2" / "outputs" / "TRKB_20chol_L002_14.2_50ps_R2_final.pdb"
CHECKPOINT_IN = PILOT_ROOT / "md50ps_R2" / "outputs" / "TRKB_20chol_L002_14.2_50ps_R2.chk"
STATE_XML_IN = PILOT_ROOT / "md50ps_R2" / "outputs" / "TRKB_20chol_L002_14.2_50ps_R2_state.xml"
RESTRAINT_ANCHOR_PDB = PILOT_ROOT / "short_md_retry_R2" / "outputs" / "TRKB_20chol_L002_14.2_shortMD_R2_final.pdb"
ORIGINAL_R2_INPUT_PDB = PILOT_ROOT / "restraint_strategy_fix" / "outputs" / "R2_pbcaware_minimized.pdb"
...
LOG_PATH = LOGS_DIR / "md100ps_R2.log"
FINAL_PDB = OUTPUTS_DIR / "TRKB_20chol_L002_14.2_100ps_R2_final.pdb"
DCD_PATH = OUTPUTS_DIR / "TRKB_20chol_L002_14.2_100ps_R2.dcd"
STATE_XML = OUTPUTS_DIR / "TRKB_20chol_L002_14.2_100ps_R2_state.xml"
CHECKPOINT_PATH = OUTPUTS_DIR / "TRKB_20chol_L002_14.2_100ps_R2.chk"
MONITOR_CSV = REPORTS_DIR / "md100ps_R2_monitor.csv"
SUMMARY_MD = REPORTS_DIR / "md100ps_R2_summary.md"
...
TIMESTEP_PS = 0.0005
TOTAL_STEPS = 200000  # 100 ps at 0.5 fs
REPORT_INTERVAL_STEPS = 2000
SAFETY_INTERVAL_STEPS = 100
DCD_INTERVAL_STEPS = 4000
...
STAGES = [
    Stage("310K_100ps", 310.0, TOTAL_STEPS),
]
...
MONITOR_FIELDS = [
    ...
    "ligand_heavy_rmsd_vs_original_R2_input_A",
    "severe_lt_1A",
    "close_lt_1p5A",
    "status",
]
...
def make_monitor_row(
    ...
    return {
        ...
        "ligand_heavy_rmsd_vs_100ps_input_A": f"{retention['ligand_rmsd_A']:.6g}" if math.isfinite(retention["ligand_rmsd_A"]) else "nan",
        "ligand_heavy_rmsd_vs_original_R2_input_A": f"{retention['ligand_rmsd_original_R2_A']:.6g}" if math.isfinite(retention["ligand_rmsd_original_R2_A"]) else "nan",
...
def stats(rows: list[dict[str, Any]], key: str) -> dict[str, float]:
    ...
    if not values:
        return {"initial": float("nan"), "final": float("nan"), "min": float("nan"), "max": float("nan"), "avg": float("nan")}
    return {
        "initial": values[0],
        "final": values[-1],
        "min": min(values),
...
def write_summary(result: dict[str, Any]) -> None:
    ...
    temp_stats = stats(rows, "actual_temperature_K")
    pe_stats = stats(rows, "potential_energy_kJ_mol")
    key_stats = stats(rows, "ligand_key_min_distance_A")
    rmsd_stats = stats(rows, "ligand_heavy_rmsd_vs_100ps_input_A")
    original_r2_rmsd_stats = stats(rows, "ligand_heavy_rmsd_vs_original_R2_input_A")
    near_chain_values = [to_int(row.get("ligand_near_chains_count"), -1) for row in rows]
    severe_values = [to_int(row.get("severe_lt_1A"), 999) for row in rows]
    close_values = [to_int(row.get("close_lt_1p5A"), 999) for row in rows]
    near_chains_always_two = bool(near_chain_values) and all(value >= 2 for value in near_chain_values)
    severe_seen = any(value > 0 for value in severe_values)
    close_seen = any(value > 0 for value in close_values)
    pe_finite = bool(numeric_values(rows, "potential_energy_kJ_mol")) and len(numeric_values(rows, "potential_energy_kJ_mol")) == len(rows)
    pe_drift = pe_stats["final"] - pe_stats["initial"] if math.isfinite(pe_stats["final"]) and math.isfinite(pe_stats["initial"]) else float("nan")
    lines = [
        ...
        "",
        f"- PSF: `{INPUT_PSF}`",
        f"- 50 ps final PDB: `{INPUT_PDB}`",
        f"- 50 ps checkpoint: `{CHECKPOINT_IN}`",
        f"- 50 ps state XML: `{STATE_XML_IN}`",
        f"- Checkpoint-compatible restraint anchor PDB: `{RESTRAINT_ANCHOR_PDB}`",
        f"- Original R2 input PDB for RMSD reference: `{ORIGINAL_R2_INPUT_PDB}`",
        f"- GRO box: `{GRO_BOX}`",
        ...
        "- Temperature: 310 K",
        f"- Total planned time: {TOTAL_STEPS * TIMESTEP_PS:.3f} ps",
        f"- Monitor interval: {REPORT_INTERVAL_STEPS} steps",
        f"- DCD interval: {DCD_INTERVAL_STEPS} steps",
        f"- Started from checkpoint: {'YES' if result.get('used_checkpoint') else 'NO'}",
        f"- Start mode: {result.get('start_mode', 'not reached')}",
        f"- Start fallback reason: {result.get('fallback_reason') or 'none'}",
        "",
        "## Outcome",
        "",
        f"- Completed 100 ps: {'YES' if result.get('completed_100ps') else 'NO'}",
        f"- Last completed step: {result.get('last_step', 0)} / {TOTAL_STEPS}",
        f"- Last completed time: {to_float(result.get('last_step'), 0.0) * TIMESTEP_PS:.3f} ps",
        ...
        f"- Temperature range / average / final: {fmt(temp_stats['min'])} to {fmt(temp_stats['max'])} K / {fmt(temp_stats['avg'])} K / {fmt(temp_stats['final'])} K",
        f"- Potential energy initial/final/min/max/average: {fmt(pe_stats['initial'])} / {fmt(pe_stats['final'])} / {fmt(pe_stats['min'])} / {fmt(pe_stats['max'])} / {fmt(pe_stats['avg'])} kJ/mol",
        f"- Potential energy finite through monitor rows: {'YES' if pe_finite else 'NO'}",
        f"- Potential energy final-minus-initial: {fmt(pe_drift)} kJ/mol",
        f"- Ligand min distance to TYR13/VAL17/SER20 initial/final/min/max/average: {fmt(key_stats['initial'])} / {fmt(key_stats['final'])} / {fmt(key_stats['min'])} / {fmt(key_stats['max'])} / {fmt(key_stats['avg'])} A",
        f"- Ligand near chains always at least 2: {'YES' if near_chains_always_two else 'NO'}",
        f"- Ligand heavy atom RMSD vs 100 ps input pose initial/final/max: {fmt(rmsd_stats['initial'])} / {fmt(rmsd_stats['final'])} / {fmt(rmsd_stats['max'])} A",
        f"- Ligand heavy atom RMSD vs original R2 input pose initial/final/max: {fmt(original_r2_rmsd_stats['initial'])} / {fmt(original_r2_rmsd_stats['final'])} / {fmt(original_r2_rmsd_stats['max'])} A",
        f"- Severe clashes appeared: {'YES' if severe_seen else 'NO'}",
        f"- Close contacts appeared: {'YES' if close_seen else 'NO'}",
        f"- Final severe/close contacts: {final_row.get('severe_lt_1A', 'nan')} / {final_row.get('close_lt_1p5A', 'nan')}",
        f"- Recommend next stage: {'YES' if result.get('recommend_next_stage') else 'NO'}",
        f"- Recommended next stage: {result.get('next_stage_recommendation', 'review monitor data before extending')}",
        "",
        "## Output Files",
        "",
        f"- Final PDB: `{FINAL_PDB}`",
        f"- DCD: `{DCD_PATH}`",
        f"- State XML: `{STATE_XML}`",
    ...
    if not result.get("recommend_next_stage"):
        lines.extend(
            [
                "",
                "## Next Step",
                "",
                "- Do not expand to 14.2 / 40chol until the monitor warnings are reviewed.",
            ]
        )
    else:
        lines.extend(
            [
                ...
                "## Next Step",
                "",
                f"- {result.get('next_stage_recommendation')}",
            ]
        )
    SUMMARY_MD.write_text("\n".join(lines) + "\n", encoding="utf-8")


def run_100ps_md() -> dict[str, Any]:
    import openmm
    ...
    restraint_anchor_pdb = PDBFile(str(RESTRAINT_ANCHOR_PDB))
    if len(restraint_anchor_pdb.positions) != len(pdb.positions):
        raise ValueError(
            f"Restraint anchor atom count mismatch: anchor {len(restraint_anchor_pdb.positions)}, input {len(pdb.positions)}"
        )
    original_r2_pdb = PDBFile(str(ORIGINAL_R2_INPUT_PDB))
    if len(original_r2_pdb.positions) != len(pdb.positions):
        raise ValueError(f"Original R2 atom count mismatch: original {len(original_r2_pdb.positions)}, input {len(pdb.positions)}")
    restraint_counts = add_r2_pbc_restraints(system, atoms, restraint_anchor_pdb.positions)
    box = read_gro_box_lengths_a()
    reference_coords = positions_to_angstrom(pdb.positions)
    original_r2_coords = positions_to_angstrom(original_r2_pdb.positions)
    ligand_heavy_indices = [index for index, atom in enumerate(atoms) if is_ligand(atom) and is_heavy(atom)]

    print("100 ps restrained MD pilot continuation R2 for 14.2 / L002 / 20chol")
    print("This is not production MD. No CUDA will be used. No longer-than-100-ps MD will be run.")
    print("R2 PBC-aware restraint expression: 0.5*k*periodicdistance(x,y,z,x0,y0,z0)^2")
    print(f"Input PSF: {INPUT_PSF}")
    print(f"50 ps final PDB: {INPUT_PDB}")
    print(f"50 ps checkpoint: {CHECKPOINT_IN}")
    print(f"50 ps state XML: {STATE_XML_IN}")
    print(f"Checkpoint-compatible restraint anchor PDB: {RESTRAINT_ANCHOR_PDB}")
    print(f"Original R2 input PDB for RMSD reference: {ORIGINAL_R2_INPUT_PDB}")
    ...
    integrator = LangevinMiddleIntegrator(310.0 * kelvin, 10.0 / picoseconds, TIMESTEP_PS * picoseconds)
    integrator.setRandomNumberSeed(142350)
    simulation = Simulation(psf.topology, system, integrator, platform)
    simulation.context.setPeriodicBoxVectors(*read_gro_box_vectors())
    simulation.context.setPositions(pdb.positions)
    used_checkpoint = False
    start_mode = "fallback_pdb_reinitialized_velocities"
    fallback_reason = ""
    try:
        simulation.context.loadCheckpoint(CHECKPOINT_IN.read_bytes())
        integrator.setTemperature(310.0 * kelvin)
        used_checkpoint = True
        start_mode = "checkpoint"
        print("Loaded 50 ps checkpoint successfully; continuing from checkpoint state.")
    except Exception as exc:
        fallback_reason = repr(exc)
        print(f"WARNING: checkpoint load failed; falling back to 50 ps final PDB with new 310 K velocities: {fallback_reason}")
        simulation.context.setPeriodicBoxVectors(*read_gro_box_vectors())
        simulation.context.setPositions(pdb.positions)
        simulation.context.setVelocitiesToTemperature(310.0 * kelvin, 142350)
    ...
    severe_clash_warning_streak = 0
    global_step = 0

    def sample(stage_name: str, target_temperature: float, force_write: bool) -> tuple[dict[str, Any], bool]:
        nonlocal last_positions, last_row, max_temperature, has_nan, key_distance_warning_streak, severe_clash_warning_streak, stop_reason
        ...
        if values_finite:
            ...
            retention["ligand_rmsd_original_R2_A"] = direct_rmsd(ligand_heavy_indices, original_r2_coords, coords)
            ...
            retention = {
                ...
                "ligand_rmsd_original_R2_A": float("nan"),
            }
        ...
        if clashes["severe_lt_1A"] > 0:
            ...
            severe_clash_warning_streak += 1
            if severe_clash_warning_streak >= 5:
                statuses.append("severe_clash_warning_persistent")
                abort = True
        else:
            severe_clash_warning_streak = 0
        if not statuses:
    ...
    sample(f"initial_{start_mode}", 310.0, True)
    for stage in STAGES:
        if stopped:
            break
    ...
    write_pdb(
        ...
        FINAL_PDB,
        [
            "REMARK 100 ps restrained MD pilot R2 final safe coordinates",
            "REMARK not production MD",
            f"REMARK completed_100ps {completed}",
            f"REMARK used_checkpoint {used_checkpoint}",
            f"REMARK start_mode {start_mode}",
            f"REMARK last_step {global_step}",
            f"REMARK last_time_ps {global_step * TIMESTEP_PS:.6g}",
            f"REMARK stop_reason {stop_reason or 'none'}",
        ],
    )
    state = simulation.context.getState(getPositions=True, getVelocities=True, getEnergy=True, getParameters=True, enforcePeriodicBox=True)
    ...
    rmsd_values = numeric_values(rows, "ligand_heavy_rmsd_vs_100ps_input_A")
    status_values = [row.get("status", "") for row in rows]
    severe_values = [to_int(row.get("severe_lt_1A"), 999) for row in rows]
    close_values = [to_int(row.get("close_lt_1p5A"), 999) for row in rows]
    near_chain_values = [to_int(row.get("ligand_near_chains_count"), 0) for row in rows]
    recommend_next_stage = (
        completed
        and not has_nan
        and math.isfinite(max_temperature)
        and max_temperature <= TEMPERATURE_ABORT_K
        and all(status == "ok" for status in status_values)
        and all(value >= 2 for value in near_chain_values)
        ...
        and all(value == 0 for value in severe_values)
        and max(close_values or [999]) == 0
        and to_float(final_row.get("ligand_heavy_rmsd_vs_100ps_input_A")) <= 5.0
    )
    next_stage_recommendation = (
        "The monitored criteria support considering a separate non-production 14.2 / 40chol pilot with the same Branch-B/R2 strategy; "
        "do not treat this as production MD."
        if recommend_next_stage
        else "Review this 100 ps monitor and final structure before expanding to 14.2 / 40chol."
    )
    result = {
        ...
        "final_row": final_row,
        "rows": rows,
        "max_ligand_rmsd_A": max(rmsd_values) if rmsd_values else float("nan"),
        "recommend_next_stage": recommend_next_stage,
        "next_stage_recommendation": next_stage_recommendation,
    }
    write_summary(result)
    print("")
    print("Terminal summary")
    print(f"completed_100ps: {completed}")
    print(f"continued_from_checkpoint: {used_checkpoint}")
    print(f"start_mode: {start_mode}")
    print(f"fallback_reason: {fallback_reason or 'none'}")
    print(f"last_step: {global_step}/{TOTAL_STEPS}")
    print(f"last_time_ps: {global_step * TIMESTEP_PS:.6g}")
    ...
    print(f"ligand_final_RMSD_vs_100ps_input_A: {final_row.get('ligand_heavy_rmsd_vs_100ps_input_A', 'nan')}")
    print(f"ligand_final_RMSD_vs_original_R2_input_A: {final_row.get('ligand_heavy_rmsd_vs_original_R2_input_A', 'nan')}")
    print(f"ligand_max_RMSD_A: {fmt(result.get('max_ligand_rmsd_A'))}")
    print(f"final_severe_close_contacts: {final_row.get('severe_lt_1A', 'nan')}/{final_row.get('close_lt_1p5A', 'nan')}")
    print(f"recommend_expand_to_14.2_40chol: {recommend_next_stage}")
    print(f"final PDB: {FINAL_PDB}")
    print(f"DCD: {DCD_PATH}")
    print(f"state XML: {STATE_XML}")
...
def failure_result(error: str) -> dict[str, Any]:
    ...
    result = {
        ...
        "used_checkpoint": False,
        "start_mode": "not reached",
        "fallback_reason": error,
        "final_row": {},
        "rows": [],
        "max_ligand_rmsd_A": float("nan"),
        "recommend_next_stage": False,
        "next_stage_recommendation": "The 100 ps pilot failed before producing monitor data; review the log before expanding to 14.2 / 40chol.",
    }
    ...
    with LOG_PATH.open("w", encoding="utf-8") as log_file:
        ...
        with redirect_stdout(tee_out), redirect_stderr(tee_err):
            try:
                ...
                error = repr(exc)
                print("FAILED: 100 ps restrained MD pilot R2 failed:", error)
                traceback.print_exc()
