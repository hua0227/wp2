...
INPUT_PDB = PILOT_ROOT / "short_md_retry_R2" / "outputs" / "TRKB_20chol_L002_14.2_shortMD_R2_final.pdb"
...
LOG_PATH = LOGS_DIR / "md50ps_R2.log"
FINAL_PDB = OUTPUTS_DIR / "TRKB_20chol_L002_14.2_50ps_R2_final.pdb"
DCD_PATH = OUTPUTS_DIR / "TRKB_20chol_L002_14.2_50ps_R2.dcd"
STATE_XML = OUTPUTS_DIR / "TRKB_20chol_L002_14.2_50ps_R2_state.xml"
CHECKPOINT_PATH = OUTPUTS_DIR / "TRKB_20chol_L002_14.2_50ps_R2.chk"
MONITOR_CSV = REPORTS_DIR / "md50ps_R2_monitor.csv"
SUMMARY_MD = REPORTS_DIR / "md50ps_R2_summary.md"
...
TIMESTEP_PS = 0.0005
TOTAL_STEPS = 100000  # 50 ps at 0.5 fs
REPORT_INTERVAL_STEPS = 500
SAFETY_INTERVAL_STEPS = 50
...
STAGES = [
    Stage("310K_50ps", 310.0, TOTAL_STEPS),
]
...
def write_summary(result: dict[str, Any]) -> None:
    ...
    rmsd_stats = stats(rows, "ligand_heavy_rmsd_A")
    ...
    lines = [
        ...
        "## Outcome",
        "",
        f"- Completed 50 ps: {'YES' if result.get('completed_50ps') else 'NO'}",
        ...
        f"- Ligand heavy atom RMSD vs 50 ps input pose initial/final/max: {fmt(rmsd_stats['initial'])} / {fmt(rmsd_stats['final'])} / {fmt(rmsd_stats['max'])} A",
    ...
    if not result.get("recommend_next_stage"):
        lines.extend(
            [
                ...
                "## Next Step",
                "",
                "- Do not move to a 100 ps restrained MD pilot or expand to 40chol / other ligands until the monitor warnings are reviewed.",
            ]
        )
    else:
        lines.extend(
...
def run_50ps_md() -> dict[str, Any]:
    ...
    print("50 ps restrained MD pilot R2 for 14.2 / L002 / 20chol")
    print("This is not production MD. No CUDA will be used. No longer-than-50-ps MD will be run.")
    ...
    sample("initial_310K", 310.0, True)
    for stage in STAGES:
        if stopped:
            break
    ...
    write_pdb(
        ...
        FINAL_PDB,
        [
            "REMARK 50 ps restrained MD pilot R2 final safe coordinates",
            "REMARK not production MD",
            f"REMARK completed_50ps {completed}",
    ...
    rmsd_values = numeric_values(rows, "ligand_heavy_rmsd_A")
    ...
    next_stage_recommendation = (
        "The monitored criteria support a non-production 100 ps restrained MD pilot with the same R2 PBC-aware restraints; "
        "40chol / other-ligand expansion can be considered after that pilot remains stable."
        if recommend_next_stage
        else "Review this 50 ps monitor and final structure before attempting 100 ps restrained MD or expanding to 40chol / other ligands."
    )
    result = {
    ...
    print(f"completed_50ps: {completed}")
    ...
    print(f"recommend_next_stage: {recommend_next_stage}")
...
def failure_result(error: str) -> dict[str, Any]:
    ...
    result = {
        ...
        "next_stage_recommendation": "The 50 ps pilot failed before producing monitor data; review the log before extending.",
    }
    ...
    with LOG_PATH.open("w", encoding="utf-8") as log_file:
        ...
        with redirect_stdout(tee_out), redirect_stderr(tee_err):
            try:
                ...
                error = repr(exc)
                print("FAILED: 50 ps restrained MD pilot R2 failed:", error)
                traceback.print_exc()
                result = failure_result(error)
    return 0 if result.get("completed_50ps") and not result.get("has_nan") else 1
