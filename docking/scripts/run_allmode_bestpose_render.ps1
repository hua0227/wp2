...
New-Item -ItemType Directory -Force -Path "$root\best_pose_review_pdbqt" | Out-Null
New-Item -ItemType Directory -Force -Path "$root\best_pose_review_pdb" | Out-Null
New-Item -ItemType Directory -Force -Path "$root\figure_best\views" | Out-Null
New-Item -ItemType Directory -Force -Path "$root\figure_best\panels" | Out-Null
New-Item -ItemType Directory -Force -Path "$root\figure_best\pml" | Out-Null

Write-Host "=== Step 1: Write all-mode analysis + best-pose extraction script ==="
...
'@ | Set-Content -Path C:\TRKB_WP2\docking\scripts\allmode_bestpose_prepare_figures.py -Encoding UTF8

Write-Host "=== Step 2: Run all-mode analysis and prepare best-pose PDBQT/PML ==="
python C:\TRKB_WP2\docking\scripts\allmode_bestpose_prepare_figures.py

Write-Host "=== Step 3: Convert selected best-pose PDBQT to PDB using Open Babel ==="

$obabelCmd = Get-Command obabel -ErrorAction SilentlyContinue
$obabelExe = $null

if ($obabelCmd) {
    $obabelExe = $obabelCmd.Source
} else {
    $condaEnv = "C:\Users\14566\miniconda3\envs\trkb_openmm"
    $candidates = @(
        "$condaEnv\Library\bin\obabel.exe",
        "$condaEnv\Scripts\obabel.exe"
    )
...
if (-not $obabelExe) {
    throw "Could not find obabel.exe. Please check Open Babel installation."
}

Write-Host "Using obabel:" $obabelExe

Get-ChildItem "$root\best_pose_review_pdbqt" -Filter *.pdbqt | ForEach-Object {
    $inFile = $_.FullName
    $outFile = Join-Path "$root\best_pose_review_pdb" ($_.BaseName + ".pdb")
    Write-Host "Converting:" $_.Name
    & $obabelExe $inFile -O $outFile
}

Write-Host "=== Step 4: Find PyMOL and render PML screenshots ==="

$candidates = @(
    "$env:ProgramFiles\PyMOL\PyMOLWin.exe",
    "$env:ProgramFiles\PyMOL\PyMOL.exe",
    "$env:ProgramFiles\Open-Source PyMOL\PyMOLWin.exe",
    "$env:ProgramFiles\Open-Source PyMOL\PyMOL.exe",
    "C:\Program Files\PyMOL\PyMOLWin.exe",
    "C:\Program Files\PyMOL\PyMOL.exe",
    "C:\Program Files\Open-Source PyMOL\PyMOLWin.exe",
    "C:\Program Files\Open-Source PyMOL\PyMOL.exe"
)
...
if (-not $pymolExe) {
    Write-Host "WARNING: Could not find PyMOL automatically."
    Write-Host "PML files have been generated here:"
    Write-Host "$root\figure_best\pml"
    Write-Host "Open PyMOL manually and run any .pml file, or edit this script to set `$pymolExe."
} else {
    Write-Host "Using PyMOL:" $pymolExe
    $pmlFiles = Get-ChildItem "$root\figure_best\pml" -Filter *.pml | Sort-Object Name
...
Write-Host "=== Step 5: Write panel-making script ==="
...
'@ | Set-Content -Path C:\TRKB_WP2\docking\scripts\make_bestpose_panels.py -Encoding UTF8

Write-Host "=== Step 6: Install/check Pillow and make panels ==="
python -c "import PIL" 2>$null
if ($LASTEXITCODE -ne 0) {
    python -m pip install pillow
}

python C:\TRKB_WP2\docking\scripts\make_bestpose_panels.py

Write-Host "=== ALL DONE ==="
Write-Host "All-mode analysis:"
Write-Host "$root\all_modes_contact_analysis.csv"
Write-Host "Best pose per ligand:"
Write-Host "$root\best_pose_per_ligand.csv"
Write-Host "Best pose PDB:"
Write-Host "$root\best_pose_review_pdb"
Write-Host "View images:"
Write-Host "$root\figure_best\views"
Write-Host "Panel images:"
Write-Host "$root\figure_best\panels"