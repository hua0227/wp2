...
$out20 = "$root\results_batch\20chol"
$out40 = "$root\results_batch\40chol"
$logDir = "$root\logs_batch"

New-Item -ItemType Directory -Force -Path $out20 | Out-Null
New-Item -ItemType Directory -Force -Path $out40 | Out-Null
New-Item -ItemType Directory -Force -Path $logDir | Out-Null

# Find vina.exe using the conda environment path first
...
if (-not $vinaExe) {
    throw "Could not find vina.exe in trkb_openmm. Run: Get-ChildItem C:\Users\14566\miniconda3\envs\trkb_openmm -Recurse -Filter vina.exe"
}
